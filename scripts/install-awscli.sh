...
if command -v aws &> /dev/null; then
    INSTALLED_VERSION=$(aws --version | awk '{print $1}' | cut -d'/' -f2)
    echo "✓ AWS CLI already installed: version $INSTALLED_VERSION"
    exit 0
fi
...
verify_awscli_binary() {
    local bin_path="/usr/local/aws-cli/v2/current/bin/aws"
    ...
    echo "Verifying AWS CLI binary..."
    if codesign -v -strict "$bin_path" &>/dev/null; then
        echo "✓ AWS CLI binary signature verified"
        return 0
    else
